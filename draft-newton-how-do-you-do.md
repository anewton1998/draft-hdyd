...
* `ah1_u32` - the unsigned 32-bit integer that is an AH1.
* `ah1_alg` - the hash algorithm used to calculate AH1.
* `created` - the date and time value taken from (#values_request).
* `ah2_u32` - the unsigned 32-bit integer that is an AH2.
* `ah2_alg` - the hash algorithm used to calculate AH2.

The CEA uses the AH1 valud and the `created` value to reference a set of user identifier hashes (UH1, UH2, and UH3).
If the AH2 value given matches the UH2 value in that set of hashes, the CEA responds the UH3 in this form:
...
If the REA can match this UH3 value its corresponding AH3 value, then the REA may send a
