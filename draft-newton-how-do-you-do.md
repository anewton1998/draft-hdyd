...
All three hash values, UH1, UH2, and UH3, are provisioned into the EA by the UA. The UID is not.
...
1. The REA sends a message containing a UH1 and UH2.
2. If both the UH1 and UH2 are associated with the same UID, the CEA confirms with the UH3 of that UID.
3. Having established confirmation, subsequent messages are possible.

# Two Way Confirmation {#two_way}

These are the steps for two-way confirmation:

1. The REA sends a message containing a UH1 and AH1.
2. If the CEA can associate the AH1 with the UH1 (that is, the AID is associated with the UID), it responds with a simple positive acknowledgement of the match.
3. Next, the REA sends the UH2 and AH2 corresponding to the UH1 and AH1 in the first message.
4. If the CEA can associate the UH2 with the UH1 of the first message and the AH2 with the AH1 of the first message, the CEA confirms by sending the UH3 and AH3.
5. Finally, the REA confirms the AID by sending a message to the CEA containing the AH3.
...
However, the protocol supports multiple messages of various types. Each type is listed in an IANA registry.
...
> This needs some more work, obviously.

## Distribution

An Exchange Agent may request knowledge of existence of other agents from a known agent
...
This invites a type of attack where an Exchange Agent may purposefully use be populated
